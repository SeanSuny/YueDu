...
def getParentId(cookie, accountIndex):
    ...
    headers = {
        "User-Agent":
        "Mozilla/5.0 (Linux; Android 9; V1923A Build/PQ3B.190801.06161913; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/91.0.4472.114 Safari/537.36 MMWEBID/5635 MicroMessenger/8.0.40.2420(0x28002837) WeChat/arm64 Weixin Android Tablet NetType/WIFI Language/zh_CN ABI/arm64",
        "Cookie": cookie,
    }
...
def main_task(accountData, accountIndex):
    ...
    try:
        ...
        if response["code"] == 0:
            ...
            print(f" 账号【{accountIndex}】ID:{myUserId}-----钢镚余额:{remain}\n今日阅读量::{read}\n推广链接:{share_link}")
        else:
            ...
            try:
                ...
                if postUrl:
                    try:
                        ...
                        try:
                            result = requests.get(url, headers=headers, timeout=7, allow_redirects=False)
                        except requests.Timeout:
                            result = requests.get(url, headers=headers, timeout=7, allow_redirects=False)
                        if result.status_code == 302:
                            ...
                            try:
                                ...
                                print(f" 账号【{accountIndex}】第{readIndex+1}次阅读失败：提不到文章参数，请联系作者帮忙更新")
                                continue
                    ...
                    if biz in biz_list:
                        print(f" 账号【{accountIndex}】第{readIndex+1}次阅读文章 >>> 发现目标[{biz}] 疑似检测文章！！！")
                        link = response["data"]["link"]
                        url = "http://wxpusher.zjiecode.com/api/send/message"
                        if wechatBussinessKey:
                            url = ("https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=" + wechatBussinessKey)
                        ...
                        for message in messages:
                            ...
                            with lock:
                                if autoSkipRead == False:
                                    print(f" 账号【{accountIndex}】不属于{myInviteId}的下级，抱歉，不执行自动阅读推送--{randomWaitTime}s后继续运行")
                                    ...
                                    if response["code"] == 0:
                                        ...
                                        print(f" 账号【{accountIndex}】第{readIndex+1}次阅读检测文章成功---获得钢镚[{gain}]")
                                        print(f"--------------------------------")
                                    else:
                                        print(f" 账号【{accountIndex}】第{readIndex+1}次阅读❗过检测失败，请尝试重新运行")
                                        break
                                else:
                                    response = requests.post(url, headers=headers, data=json.dumps(data))
                                    print(f" 账号【{accountIndex}】已将第{readIndex+1}篇文章推送至微信请在{randomWaitTime}s内点击链接完成阅读--{randomWaitTime}s后继续运行")
                                    time.sleep(randomWaitTime)
                                    url = ("http://2568134.9o.10r8cvn6b1.cloud/read/finish")
                                    headers = {
                                        "User-Agent":
                        ...
                        try:
                            ...
                            print(f" 账号【{accountIndex}】第{readIndex+1}次完成阅读❗请求超时，尝试重新发送请求...")
                            response = requests.get(url, headers=headers, data=data, timeout=7).json()
                        if response["code"] == 0:
                            gain = response["data"]["gain"]
                            print(f" 账号【{accountIndex}】第{readIndex+1}次阅读文章成功---获得钢镚[{gain}]")
                            print(f"--------------------------------")
                        else:
                            print(f" 账号【{accountIndex}】❗第{readIndex+1}次阅读文章失败{response}")
                            break
                else:
